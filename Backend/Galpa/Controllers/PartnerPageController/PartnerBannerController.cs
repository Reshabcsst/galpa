...
namespace Galpa.Controllers.PartnerPageController
{
    ...
    [ApiController]
    public class PartnerBannerController : ControllerBase
    {
        ...
        public PartnerBannerController(IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> AddBannerItem([FromForm] BannerModel banner)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                int newId = 1;
                using (MySqlCommand cmd = new MySqlCommand("SELECT MAX(Id) FROM partnerbanner", con))
                {
                ...
                using (MySqlCommand cmd = new MySqlCommand("INSERT INTO partnerbanner (Id, Heading, Subheading, BackgroundImage) VALUES (@Id, @Heading, @Subheading, @BackgroundImage)", con))
                {
        ...
        public IActionResult GetBannerItem()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("SELECT * FROM partnerbanner", con))
                {
        ...
        public async Task<IActionResult> EditBannerItem(int id, [FromForm] BannerModel banner)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlCommand getCmd = new MySqlCommand("SELECT Heading, Subheading, BackgroundImage FROM partnerbanner WHERE Id = @Id", con))
                {
                ...
                using (MySqlCommand cmd = new MySqlCommand("UPDATE partnerbanner SET Heading = @Heading, Subheading = @Subheading, BackgroundImage = @BackgroundImage WHERE Id = @Id", con))
                {
        ...
        [Authorize]
        [HttpDelete]
        [Route("delete-partner-banner/{id}")]
        public IActionResult DeleteBannerItem(int id)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("DELETE FROM partnerbanner WHERE Id = @Id", con))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
