...
namespace Galpa.Controllers.ServicePageController
{
    ...
    public class ServiceBannerController : ControllerBase
    {
        ...
        public ServiceBannerController(IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> AddBannerItem([FromForm] BannerModel banner)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                int newId = 1;
                using (MySqlCommand cmd = new MySqlCommand("SELECT MAX(Id) FROM ServiceBanner", con))
                {
                ...
                using (MySqlCommand cmd = new MySqlCommand("INSERT INTO ServiceBanner (Id, Heading, Subheading, BackgroundImage) VALUES (@Id, @Heading, @Subheading, @BackgroundImage)", con))
                {
       ...
       // [Authorize]
        ...
        public IActionResult GetBannerItem()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("SELECT * FROM ServiceBanner", con))
                {
        ...
        public async Task<IActionResult> EditBannerItem(int id, [FromForm] BannerModel banner)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlCommand getCmd = new MySqlCommand("SELECT Heading, Subheading, BackgroundImage FROM ServiceBanner WHERE Id = @Id", con))
                {
                ...
                using (MySqlCommand cmd = new MySqlCommand("UPDATE ServiceBanner SET Heading = @Heading, Subheading = @Subheading, BackgroundImage = @BackgroundImage WHERE Id = @Id", con))
                {
        ...
        [Authorize]
        [HttpDelete]
        [Route("delete-service-banner/{id}")]
        public IActionResult DeleteBannerItem(int id)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("DELETE FROM ServiceBanner WHERE Id = @Id", con))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
